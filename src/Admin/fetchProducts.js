...
const FetchProducts = () => {
  ...
  const [heldProducts, setHeldProducts] = useState([]); // New state for held products
  ...
  const fetchHeldProducts = async () => {
    try {
      ...
      if (snapshot.exists()) {
        ...
        // If no held products exist, set empty array
        setHeldProducts([]);
        console.log("No held products found in database");
      }
    } catch (error) {
      console.error("Error fetching held products:", error);
      // Set empty array instead of showing error for non-existent collection
      setHeldProducts([]);
      // Only show error message if it's a real database connection issue
      if (error.code !== 'PERMISSION_DENIED') {
        setErrorMessage("Error fetching held products. Please try again.");
        setTimeout(() => setErrorMessage(null), 3000);
      }
    }
  };
...
const handleHoldProduct = async (product) => {
  const confirmed = window.confirm(
    `Are you sure you want to hold the product "${product.name}"?`
  );
  if (!confirmed) return;
  
  try {
    // Clean the product data before saving to Firebase
    const cleanedProduct = {
      name: product.name || '',
      productType: product.productType || '',
      itemCost: product.itemCost !== null && product.itemCost !== undefined ? product.itemCost : 0,
      purchasingPrice: product.purchasingPrice !== null && product.purchasingPrice !== undefined ? product.purchasingPrice : 0,
      quantity: product.quantity || 0,
      heldDate: new Date().toISOString()
    };
    
    // Add to held products
...
const handleRestoreProduct = async (product) => {
  ...
  try {
    ...
    // Remove from held products
    const heldProductRef = ref(database, `heldProducts/${product.id}`);
    await remove(heldProductRef);
    
    // Update local state
    setHeldProducts(heldProducts.filter((p) => p.id !== product.id));
                ...
                <td>
                  {editingProduct && editingProduct.id === product.id ? (
                    ...
                    <>
                      ...
                      <button
                        onClick={() => handleHoldProduct(product)}
                        className="admin-button hold-button"
                      >
                        Hold
                      </button>
                    </>
  ...
  const renderHoldsTab = () => (
    <div className="admin-products">
        ...
        <tbody>
          {heldProducts.map((product) => (
            <tr key={product.id}>
              ...
              <td>{product.heldDate ? new Date(product.heldDate).toLocaleDateString() : 'N/A'}</td>
              <td>
                <button
                  onClick={() => handleRestoreProduct(product)}
                  className="admin-button restore-button"
                >
                  Restore
                </button>
              </td>
      ...
      {heldProducts.length === 0 && (
        <p style={{ textAlign: 'center', marginTop: '20px' }}>No held products found.</p>
      )}
    </div>
  );
    ...
    <div className="admin-container">
      ...
      <div className="tab-navigation" style={{ marginBottom: '20px' }}>
        <button
          ...
          className={`admin-button ${activeTab === 'products' ? 'active-tab' : ''}`}
          style={{
        ...
        <button
          ...
          className={`admin-button ${activeTab === 'holds' ? 'active-tab' : ''}`}
          style={{
            backgroundColor: activeTab === 'holds' ? '#007bff' : '#6c757d'
          }}
        >
          Holds ({heldProducts.length})
        </button>
      </div>
